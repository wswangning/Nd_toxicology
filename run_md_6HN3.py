"""
Nd(III) MD — 6HN3 only, 0.2ns fast run
"""
...
print("=" * 60)
print("Nd(III) MD — 6HN3 only (0.2ns)")
print("=" * 60)
...
simulation.reporters.append(app.DCDReporter(traj_file, 5000))
...
print("  生产 (0.2ns, 100000 steps)...")
t1 = time.time()
simulation.step(100000)
elapsed = time.time() - t1
print(f"  MD耗时: {elapsed:.1f}s ({100000/elapsed:.0f} steps/s)")
...
traj = md.load(traj_file, top=init_pdb)
...
time_ns = np.arange(traj.n_frames) * 0.01
...
name = dr[pdb_id]["name"]
fig.suptitle(f'MD Simulation ({name}, {pdb_id}) — 0.2ns, TIP3P', fontweight='bold', fontsize=12)
plt.tight_layout()
...
ax = axes[1]
ax.text(0.5, 0.5, 'Energy log parsing skipped', ha='center', va='center', transform=ax.transAxes)
