...
ND_SIGMA_NM = 0.263
ND_EPSILON_KJ = 0.50  # kJ/mol
ND_CHARGE = 3.0

print("=" * 60)
print("Nd(III) Vacuum MD (ultra-fast)")
print("=" * 60)
t_total = time.time()
...
for pdb_id in ["6HN3", "4IQK"]:
 ...
 <NonbondedForce coulomb14scale="0.833333" lj14scale="0.5">
  <Atom type="Nd" charge="{ND_CHARGE}" sigma="{ND_SIGMA_NM}" epsilon="{ND_EPSILON_KJ}"/>
    ...
    traj_file = f"md_trajectories/{pdb_id}_vac_traj.dcd"
    simulation.reporters.append(app.DCDReporter(traj_file, 1000))
    ...
    if not _os.path.exists(traj_file):
        print(f"  ✗ DCD文件未生成: {traj_file}")
        continue
    fsize = _os.path.getsize(traj_file)
    print(f"  DCD: {fsize} bytes")
    ...
    time_ns = np.arange(traj.n_frames) * 0.002  # 2fs * 1000 interval
    ...
    name = dr[pdb_id]["name"]
    fig.suptitle(f'MD ({name}, {pdb_id}) — Vacuum, 0.1ns', fontweight='bold', fontsize=12)
    plt.tight_layout()
    ...
    ax = axes[1]
    ax.text(0.5, 0.5, 'Energy unavailable\n(vacuum conditions)', ha='center', 
