"""
Nd(III) MD模拟 — 显式溶剂极小水盒版
=====================================
使用amber14-all.xml + TIP3P，padding=0.5nm，1ns生产模拟
"""
...
ND_SIGMA_NM = 0.263
ND_EPSILON_KCAL = 0.50 / 4.184  # 0.1195 kcal/mol
ND_CHARGE = 3.0

print("=" * 60)
print("Nd(III) MD — Explicit Solvent (minimal box)")
print("=" * 60)
...
def build_system(pdb_id):
    ...
    # 极小水盒 padding=0.5nm
    print(f"  添加水盒 (padding=0.5nm)...")
    modeller.addSolvent(forcefield, model='tip3p', padding=0.5*unit.nanometer,
    ...
    for atom in modeller.topology.atoms():
        if atom.name == "ND" and atom.residue.name == "ND":
            nb_force.setParticleParameters(
                ...
                ND_EPSILON_KCAL * 4.184 * unit.kilojoule_per_mole
            )
            nd_found = True
            print(f"  Nd参数: q={ND_CHARGE}, σ={ND_SIGMA_NM}nm, ε={ND_EPSILON_KCAL:.4f}kcal/mol")
            break
    ...
    return system, modeller.topology, modeller.positions, init_pdb
...
# ============================================================
# Step 3: 运行MD (1ns per protein)
# ============================================================
print("\n" + "=" * 60)
print("Step 3: 运行MD模拟 (1ns)")
print("=" * 60)

def run_md(pdb_id, system, topology, positions, steps=500000):
    print(f"\n--- {pdb_id} ---")
...
for pdb_id, (system, topology, positions, init_pdb) in md_systems.items():
    try:
        sim, traj, log, elapsed = run_md(pdb_id, system, topology, positions, steps=500000)
...
for pdb_id, result in md_results.items():
    ...
    time_ns = np.arange(traj.n_frames) * 0.01  # 10ps per frame
    
    # 保存数据
    np.savez(f"md_analysis/{pdb_id}_analysis.npz",
             time_ns=time_ns,
    ...
    ax.bar(residues, rmsf * 10, width=1.0, color='steelblue', edgecolor='none')
    ...
    fig.suptitle(f'MD Simulation ({name}, {pdb_id}) — 1ns, TIP3P', fontweight='bold', fontsize=12)
    plt.tight_layout()
    ...
    try:
        ...
        if pot_e is not None:
            pot_time = np.arange(len(pot_e)) * 0.01
